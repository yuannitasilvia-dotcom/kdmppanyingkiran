...
import { WISATA_TYPE_LABELS, type WisataType } from '../types';

const types: { value: WisataType | 'all'; label: string }[] = [
  { value: 'all', label: 'Semua' },
  ...Object.entries(WISATA_TYPE_LABELS).map(([value, label]) => ({
...
export default function WisataPage() {
  const [type, setType] = useState<WisataType | 'all'>('all');
  const [search, setSearch] = useState('');
  const [searchInput, setSearchInput] = useState('');
  const { items, loading, error } = useWisata({ type, search });
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-4 py-6 sm:py-8">
      <div className="mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Wisata & Homestay</h1>
        <p className="text-sm text-gray-500 mt-1">
          Jelajahi destinasi wisata dan penginapan homestay di desa-desa sekitar
        </p>
      </div>
      ...
      {!loading && items.length === 0 && (
        <div className="text-center py-16 text-gray-500">
          <p className="font-medium">Destinasi tidak ditemukan</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((item) => (
            <Link
              key={item.id}
              to={`/wisata/${item.id}`}
              className="bg-white rounded-xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md hover:ring-2 hover:ring-green-100 transition group"
            >
              ...
              <div className="p-4">
                ...
                <p className="text-xs text-gray-400 mt-0.5">{item.village}</p>
                <p className="text-red-600 font-bold text-sm mt-2">
                  {formatPrice(item.price)}{' '}
                  <span className="text-gray-400 font-normal">/ {item.price_label}</span>
                </p>
